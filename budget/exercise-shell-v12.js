(function () {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      html body .goals-grid>.goal-vo2-chart-v12{display:block!important;min-width:0;padding:18px;align-self:stretch}
      html body .goal-vo2-chart-v12 h3{margin-bottom:10px;color:#34D399}
      html body .goal-vo2-chart-v12 .bw-row{display:none!important}
      html body .goal-vo2-chart-v12 .chart-area{height:190px}

      .vo2-goal-line-v12{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:0 0 10px;padding:6px 0 7px;border-bottom:1px solid rgba(255,255,255,.055)}
      .vo2-goal-line-v12 label{color:#FBBF24;font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:.55px;white-space:nowrap}
      .vo2-goal-line-v12 input{width:62px;height:30px;padding:4px 7px;border:1px solid rgba(251,191,36,.28);border-radius:7px;background:rgba(251,191,36,.055);color:#FBBF24;outline:none;text-align:center;font:750 11px/1 'Inter',sans-serif}
      .vo2-goal-line-v12 input:focus{border-color:rgba(251,191,36,.62);box-shadow:0 0 0 2px rgba(251,191,36,.08)}
      ...
      #day-workout-modal #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12{min-width:132px!important;min-height:0!important;padding:0!important;border:0!important;border-radius:0!important;background:transparent!important;display:flex!important;align-items:flex-start!important;justify-content:flex-end!important;gap:12px!important}
      .builder-toggle-unit-v12{display:flex;flex-direction:column;align-items:center;justify-content:flex-start;gap:5px;min-width:55px}
      .builder-toggle-label-v12{color:#8B949E;font-size:8px;line-height:1.05;font-weight:800;letter-spacing:.25px;text-align:center;white-space:nowrap}
      #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12 .between-switch-v7,
      #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12 .pretimer-switch{width:40px!important;height:22px!important;padding:2px!important;margin:0!important;flex:0 0 auto!important}
      #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12 .between-switch-v7::after,
      #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12 .pretimer-switch::after{width:16px!important;height:16px!important}
      #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12 .between-switch-v7[aria-pressed="true"]::after,
      #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12 .pretimer-switch[aria-pressed="true"]::after{transform:translateX(18px)!important}
      ...
      @media(max-width:768px){
        ...
        html body .goal-vo2-chart-v12 .chart-area{height:205px}
      }
      @media(max-width:600px){
        .week-toolbar{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;align-items:center!important;gap:6px!important;width:100%!important}
        ...
        .week-toolbar .week-pick{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;align-items:center!important;gap:5px!important;flex:1 1 0!important;min-width:0!important;width:auto!important;max-width:none!important}
        .week-toolbar .week-pick input[type="date"]{flex:1 1 88px!important;min-width:0!important;width:88px!important;padding:6px 5px!important;font-size:10px!important}
        ...
        #day-workout-modal #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12{grid-column:2!important;min-width:124px!important;gap:8px!important}
      ...
      @media(max-width:380px){
        ...
        #day-workout-modal #between-exercise-toggle-panel-v7.builder-toggle-cluster-v12{grid-column:2!important;min-width:118px!important;gap:6px!important}
  ...
  function text(value){return String(value||'').trim().toLocaleLowerCase('sv-SE')}
  ...
  function relocateWeekActions(){
    ...
    if(document.getElementById('week-inline-actions-v2'))return;
    ...
    var buttons=Array.prototype.slice.call(header.querySelectorAll('button')).filter(function(button){
      ...
      return label==='redigera'||label==='mallpass';
    });
  ...
  function ensureGoalControl(card){
    if(!card)return;
    if(document.getElementById('vo2-goal-line-v12')){syncGoalInput();return}
    var row=card.querySelector('.bw-row');
    if(!row)return;
  ...
  function arrangeGoals(){
    ...
    var legacy=document.getElementById('g3-goal');
    var legacyCard=legacy&&legacy.closest?legacy.closest('.goal-card'):null;
    ...
    card.classList.add('goal-vo2-chart-v12');
  ...
  function workoutVo2Data(){
    ...
    return (Array.isArray(workouts)?workouts:[])
      .filter(function(workout){var value=Number(workout&&workout.vo2);return workout&&workout.date&&isFinite(value)&&value>=10&&value<=100})
  ...
  function installRenderSource(){
    ...
    window.renderCharts=function(){
      ...
      db.get=function(key){if(key==='vo2')return workoutVo2Data();return originalGet.call(db,key)};
  ...
  function registerGoalPlugin(){
    if(window.__exerciseVo2GoalLineV12Registered)return true;
    ...
    window.Chart.register({
      ...
      afterDatasetsDraw:function(chart){
        ...
        ctx.beginPath();ctx.moveTo(area.left,Math.round(y)+.5);ctx.lineTo(area.right,Math.round(y)+.5);ctx.stroke();ctx.setLineDash([]);
        ...
        ctx.fillStyle='rgba(22,27,34,.88)';ctx.fillRect(x-3,labelY-9,width+6,12);
        ctx.fillStyle='#FBBF24';ctx.fillText(label,x,labelY);ctx.restore();
      }
    });
    return true;
  }
    ...
    try{
      ...
      chart.data.datasets.forEach(function(dataset){
        if(dataset&&dataset.label==='Mål'){
          dataset.label='Mål VO₂ max';dataset.borderDash=[6,4];dataset.borderWidth=2;changed=true;
        }
      });
  ...
  function compactBuilderControls(){
    ...
    if(panel.dataset.compactTogglesV12==='1')return true;

    betweenButton.remove();timerButton.remove();panel.innerHTML='';
    panel.classList.add('builder-toggle-cluster-v12');panel.dataset.compactTogglesV12='1';
    ...
    betweenUnit.appendChild(betweenButton);
    ...
    panel.appendChild(betweenUnit);panel.appendChild(timerUnit);
    return true;
  }

  function scheduleBuilderControls(){
    builderTimers.forEach(clearTimeout);
    builderTimers=[0,60,160,320,560,850].map(function(delay){return setTimeout(compactBuilderControls,delay)});
  }
  ...
  function install(){
    ...
    addStyles();
    document.addEventListener('click',scheduleBuilderControls,false);
    document.addEventListener('change',function(event){if(event.target&&event.target.id==='day-workout-date')scheduleBuilderControls()},false);
    prepare();
    window.__exerciseShellV12={prepare:prepare,arrangeGoals:arrangeGoals,relocateWeekActions:relocateWeekActions,compactBuilderControls:compactBuilderControls,workoutVo2Data:workoutVo2Data};
  }
