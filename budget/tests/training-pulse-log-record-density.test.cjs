...
test('Pulse records remain 3 columns with readable category headings and rows',()=>{
 assert.match(css,/#pulse-home \.record-row-v52\{[^}]*min-height:44px!important;[^}]*padding:8px 0!important/);
...
test('Pulse log cards preserve all six cells with consistent spacing',()=>{
 assert.match(css,/#pulse-home \.log-main-row\{[^}]*padding:10px 0;/);
...
test('Pulse mobile log has two rows and records retain progress',()=>{
 ...
 assert.match(css,/#pulse-home \.record-row-v52\{grid-template-columns:minmax\(0,1fr\) 80px!important;min-height:44px!important/);
