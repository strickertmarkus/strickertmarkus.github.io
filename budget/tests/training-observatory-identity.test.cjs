...
test('boot keeps the current shared wellness ownership fresh and CP10 routes dashboard JS to one owner', () => {
  assert.match(html, /auth-config\.js\?v=20260926-no-builder-pretimer-7/);
  assert.match(html, /auth-gate\.js\?v=20260926-no-builder-pretimer-6/);
  ...
  assert.match(authConfig, /exerciseFastVersion = '20260926-pulse-compact-no-builder-timer-5'/);
  assert.match(authGate, /exerciseAssetsVersion = '20260926-pulse-compact-no-builder-timer-5'/);
});
