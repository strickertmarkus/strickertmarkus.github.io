...
test('boot keeps the current shared wellness ownership fresh and CP10 routes dashboard JS to one owner', () => {
  assert.match(html, /auth-config\.js\?v=20260920-training-only-3/);
  assert.match(html, /auth-gate\.js\?v=20260920-training-only-2/);
  ...
  assert.match(authConfig, /exerciseFastVersion = '20260920-training-only-3'/);
  assert.match(authGate, /exerciseAssetsVersion = '20260920-training-only-3'/);
});
