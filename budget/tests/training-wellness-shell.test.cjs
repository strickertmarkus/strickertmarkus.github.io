...
test('embedded original builder stays lightweight and event driven',()=>{
  ...
  assert.match(pulseFieldJs,/function openOriginalCompact\(\)/);
  assert.match(pulseFieldJs,/archive\/exercise\.html[\s\S]*overview[\s\S]*compactHost/);
  assert.match(pulseFieldJs,/pulse-field:exit-compact/);
  assert.doesNotMatch(pulseFieldJs,/fieldLayoutKey|data\.fieldLayout/);
