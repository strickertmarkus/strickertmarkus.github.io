/* Zen v47 — structural pond/cliff renderer with the original subtle cliff reflection.
   Rear stones stay in the solid scene source, while the reflection is rendered from
   a separate cliff-only source so background stones never bleed into the water. */
(function(){
  'use strict';
  ...
  function rock(cx,cy,rx,ry,lean,kind,shape){
    ...
    ctx.strokeStyle='rgba(241,239,196,.24)';ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(-rx*.72,-ry*.34);ctx.lineTo(-rx*.49,-ry*.56);ctx.lineTo(-rx*.14,-ry*.45);ctx.stroke();ctx.restore();
  }
  ...
  function drawCliffPlant(ctx,g,time){
    var s=g.s,px=g.x+45*s,py=g.crest+12,sway=reduced.matches ? 0 : Math.sin(time*.55)*2.2*s;
  ...
  var reflection=document.createElement('canvas');reflection.width=1200;reflection.height=640;
  var reflectionContext=reflection.getContext('2d');
  ...
  window.ZenPondRocks={
    draw:function(context,viewport,time){
      ...
      // Cliff-only source: this is what the checkpoint reflection sampled.
      ctx=reflectionContext;ctx.clearRect(0,0,reflection.width,reflection.height);
      ctx.save();ctx.beginPath();ctx.rect(0,0,1200,g.water);ctx.clip();drawFormation(g,time);ctx.restore();

      // Solid scene source: rear stones first, then the opaque cliff on top.
      ctx=sourceContext;ctx.clearRect(0,0,source.width,source.height);
      ctx.save();ctx.beginPath();ctx.rect(0,0,1200,g.water);ctx.clip();drawRearBank(g);ctx.drawImage(reflection,0,0);ctx.restore();

      ctx=context;ctx.save();

      // Restored verbatim from checkpoint 9464582a9c221ff08fa0ede683f5be6eeb41795f.
      // Only the cliff-only reflection canvas is sampled, never the new rear stones.
      ...
      for(var y=0;y<depth;y+=1.5){
        ...
        ctx.drawImage(reflection,sx,sy,sw,1.5/compression,sx+wave,g.water+y,sw,1.6);
      }

      ctx.globalAlpha=1;ctx.drawImage(source,0,0);
      drawImpact(g.x+18*g.s,g.water,g.s,time);ctx.restore();
      contactSeam(context,g.x-61*g.s,g.crest+23,18*g.s,-.14);
      contactSeam(context,g.x+72*g.s,g.crest+17,17*g.s,.12);
