(function(){
  ...
  const copy={
    stretch:{eyebrow:'EN LITEN GLÄNTA I VARDAGEN',title:'Låt kroppen<br><em>landa.</em>',description:'Mjuka rörelser. Djupa andetag.<br>En stund där ingenting behöver skyndas på.',symbol:'✧',caption:'SKOGSGLÄNTAN<br><small>Slå rot. Andas ut.</small>',hint:'Hitta en plats där du kan röra dig bekvämt.',growth:'Små stunder slår rot',growthEyebrow:'DET SOM FÅR VÄXA',collection:'Din glänta växer',footer:'Lite mjukare, en stund i taget.',home:'← Till gläntan'},
    meditation:{eyebrow:'ETT STILLA RUM INOM DIG',title:'Låt tankarna<br><em>flyta.</em>',description:'Ljuset över vattnet. Ett andetag i taget.<br>Här får du bara vara en stund.',symbol:'≈',caption:'VATTENTRÄDGÅRDEN<br><small>Kom till ro. Bara var.</small>',hint:'Sitt eller ligg bekvämt. Resten kan vänta.',growth:'Ringar på stilla vatten',growthEyebrow:'LUGNET FÅR TA PLATS',collection:'Din stilla trädgård',footer:'Ett andetag. En ny början.',home:'← Till trädgården'}
  };
  ...
  function setKind(next){kind=next;allHistory=false;document.body.dataset.kind=kind;document.querySelector('meta[name=theme-color]').content=kind==='stretch'?'#091d18':'#f2f1e8';const c=copy[kind];
    $('hero-eyebrow').textContent=c.eyebrow;$('hero-title').innerHTML=c.title;$('hero-description').innerHTML=c.description;$('selected-symbol').textContent=c.symbol;$('scene-caption').innerHTML=c.caption;$('start-hint').textContent=c.hint;$('growth-title').textContent=c.growth;$('growth-eyebrow').textContent=c.growthEyebrow;$('collection-title').textContent=c.collection;$('footer-copy').textContent=c.footer;$('leave-session').textContent=c.home;
  ...
  function renderHome(){
    const r=chosen();$('selected-name').textContent=r.name;$('selected-meta').textContent=mins(M.duration(r))+' min · '+(kind==='stretch'?r.steps.length+' mjuka rörelser':r.guidance==='breath'?'guidad andning':'öppen närvaro');
    const symbols=kind==='stretch'?['✧','⌁','⋮']:['≈','◌','○'];
    $('routines').innerHTML=routines().filter(r=>r.kind===kind).map((r,i)=>'<article class="ritual-card"><button class="ritual-choice" data-select="'+escape(r.id)+'" aria-pressed="'+(r.id===chosen().id)+'"><span class="ritual-top"><span class="ritual-glyph" aria-hidden="true">'+symbols[i%3]+'</span><span class="ritual-time">'+mins(M.duration(r))+' MIN'+(i>2?' · DIN RITUAL':'')+'</span></span><strong>'+escape(r.name)+'</strong><span class="ritual-description">'+escape(r.description||(kind==='stretch'?'Ditt eget mjuka flöde.':'En stund i ditt eget tempo.'))+'</span></button>'+(i>2?'<div class="ritual-tools"><button class="text-button" data-edit="'+escape(r.id)+'">Redigera</button><button class="text-button" data-remove="'+escape(r.id)+'">Ta bort</button></div>':'')+'</article>').join('');
    const st=M.stats(records(),kind,Date.now()),weekly=st.week.reduce((n,d)=>n+d.minutes,0);$('weekly-minutes').innerHTML=escape(Number(weekly.toFixed(1)).toLocaleString('sv-SE'))+'<small>minuter</small>';$('weekly-copy').textContent=weekly?'Tid du har gett till dig själv.':'Det finns plats för din första stund.';$('total-minutes').textContent=st.minutes;$('total-sessions').textContent=st.count;$('streak-days').textContent=st.streak;
    ...
    const milestones=kind==='stretch'?[['✧','Första ljuset','1 sparad stund',st.count,1],['⌁','Mjuka rötter','5 sparade stunder',st.count,5],['✦','En glänta av lugn','60 minuter totalt',st.minutes,60],['∞','Egen rytm','10 olika dagar',st.days,10]]:[['◌','Första droppen','1 sparad stund',st.count,1],['≈','Stilla ringar','5 sparade stunder',st.count,5],['☼','Morgonljus','60 minuter totalt',st.minutes,60],['∞','En lugn plats','10 olika dagar',st.days,10]];
    $('collection').innerHTML=milestones.map(([symbol,name,desc,value,goal])=>'<article class="keepsake '+(value>=goal?'unlocked':'')+'"><span class="keepsake-symbol" aria-hidden="true">'+symbol+'</span><h3>'+name+'</h3><p>'+desc+'</p><small>'+(value>=goal?'En del av din samling':Math.min(value,goal)+' / '+goal)+'</small></article>').join('');
  ...
  function showView(next){view=next;for(const name of ['home','session','complete'])$(name+'-view').hidden=name!==next;document.body.classList.toggle('in-session',next!=='home');if(next==='home'){document.body.classList.remove('is-paused');document.title='Zen · En stund för dig';}window.scrollTo(0,0);}
  ...
  function startSession(){
    if(!S.ready)return;
    if(S.active){confirm('En stund väntar redan','Fortsätt din pågående stund från startsidan, eller släpp den för att börja en ny.','Börja en ny',()=>{S.setActive(null);startSession();});return;}
  ...
  function tick(){
    ...
    if(isStretch&&lastStep!==p.index){lastStep=p.index;const step=session.routine.steps[p.index];$('session-overline').textContent='RÖRELSE '+(p.index+1)+' AV '+session.routine.steps.length;$('session-title').textContent=step.name;$('session-cue').textContent=step.cue;$('next-step').textContent=p.index+1<session.routine.steps.length?'Sedan · '+session.routine.steps[p.index+1].name:'Efter det här får du ta med dig lugnet.';if(p.index>0)chime();}
    if(!isStretch){$('session-overline').textContent='DIN STILLA STUND';$('session-title').textContent=session.routine.guidance==='breath'?'Ett andetag i taget':'Bara var här';$('session-cue').textContent=session.routine.guidance==='breath'?'Följ vattnets mjuka rytm, om det känns bekvämt. Låt andetagen komma utan ansträngning.':'Känn underlaget. Lägg märke till andetagen som de är. När tankarna vandrar får du mjukt återvända.';
    ...
    const phase=session.paused?'En liten paus':guided?(cycle<4?'Andas in':'Andas ut'):isStretch?'Låt det ta tid':'I din egen takt';
    ...
    $('session-clock').textContent=fmt(p.remaining);$('clock-caption').textContent=isStretch?'kvar i rörelsen':'kvar av din stund';$('session-elapsed').textContent=fmt(Math.floor(p.spent))+' / '+fmt(p.total);$('session-progress-fill').style.width=(p.spent/p.total*100)+'%';document.querySelector('.session-progress').setAttribute('aria-valuenow',String(Math.round(p.spent/p.total*100)));
  ...
  function complete(early){
    ...
    if(seconds<1){toast('Ge dig själv en liten stund först.');session=M.resume(session,Date.now());S.setActive(session);return;}
    ...
    $('complete-summary').textContent=mins(seconds)+' minuter · '+session.routine.name;$('session-note').value='';document.querySelectorAll('[data-feeling]').forEach(b=>b.setAttribute('aria-pressed','false'));showView('complete');$('complete-title').focus();chime();document.title='Ta med dig lugnet · Zen';
  }
  function saveComplete(){
    if(!pendingRecord)return;const record={...pendingRecord,note:$('session-note').value.trim(),updatedAt:Date.now()};
    if(S.put(record)){pendingRecord=null;session=null;S.setActive(null);showView('home');renderHome();$('hero-title').setAttribute('tabindex','-1');$('hero-title').focus();toast('Din stund är sparad.');}
  }
  ...
  function openBuilder(r,isNew){builder=JSON.parse(JSON.stringify(r));builderExisting=!isNew&&!M.routines.some(p=>p.id===r.id);if(!builderExisting)builder.id=id();$('routine-name').value=isNew?'Min '+(kind==='stretch'?'skogsstund':'stilla stund'):r.name;$('builder-error').textContent='';$('stretch-builder').hidden=kind!=='stretch';$('meditation-builder').hidden=kind!=='meditation';$('meditation-minutes').value=builder.seconds?builder.seconds/60:10;$('meditation-guidance').value=builder.guidance||'breath';$('pose-select').innerHTML=M.poses.map(p=>'<option value="'+p.id+'">'+escape(p.name)+'</option>').join('');renderBuilder();$('builder-dialog').showModal();}
  ...
  $('routines').onclick=e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.select){selected[kind]=b.dataset.select;renderHome();}if(b.dataset.edit){const r=routines().find(r=>r.id===b.dataset.edit);if(r)openBuilder(r,false);}if(b.dataset.remove){const rid=b.dataset.remove;confirm('Ta bort ritualen?','Dina redan sparade stunder finns kvar.','Ta bort',()=>S.remove(rid));}};
  ...
  $('builder-form').onsubmit=e=>{e.preventDefault();builder.name=$('routine-name').value.trim();if(builder.kind==='meditation'){builder.seconds=Number($('meditation-minutes').value)*60;builder.guidance=$('meditation-guidance').value;}if(!M.routineValid(builder)){$('builder-error').textContent='Ge ritualen ett namn och välj giltiga tider för alla delar.';return;}if(S.put({id:builder.id,type:'routine',routine:builder,updatedAt:Date.now()})){selected[kind]=builder.id;$('builder-dialog').close();renderHome();toast(builderExisting?'Din ritual är uppdaterad.':'Din egen ritual är sparad.');}else $('builder-error').textContent='Vänta tills ditt konto har laddats.';};
  ...
  $('discard-button').onclick=()=>confirm('Släppa den pågående stunden?','Den sparas inte i dina avtryck.','Släpp stunden',()=>S.setActive(null));
  ...
  $('finish-session').onclick=()=>{session=M.pause(session,Date.now());S.setActive(session);tick();if(view!=='session')return;confirm('Runda av din stund?','Du kan spara tiden du har gett dig själv hittills.','Runda av',()=>complete(true));};
  ...
  $('save-session').onclick=saveComplete;$('cancel-complete').onclick=()=>confirm('Lämna utan att spara?','Den här stunden läggs inte till i din historik.','Lämna',()=>{session=null;pendingRecord=null;S.setActive(null);showView('home');renderHome();});
  $('history').onclick=e=>{const b=e.target.closest('[data-delete]');if(b)confirm('Ta bort den här stunden?','Den tas också bort från dina framsteg och din samling.','Ta bort',()=>S.remove(b.dataset.delete));};$('history-more').onclick=()=>{allHistory=!allHistory;renderHome();};
  ...
  $('profile-name').textContent=(S.profile==='maja'?'Majas':'Markus')+' plats för lugn';
