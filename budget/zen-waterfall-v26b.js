/* Zen meditation waterfall v26b: smaller layered rocks, per-rock streamlines, lower source, clearer splash. */
(function(){
  'use strict';
  ...
  function cascade(waterY,g,time,mobile){
    ...
    for(var i=0;i<count;i++){
      var t=count===1?.5:i/(count-1),sx=g.holeX-topSpread*.5+t*topSpread,ex=g.holeX-bottomSpread*.5+t*bottomSpread;
    ...
    ctx.save();ctx.globalCompositeOperation='screen';
    for(var d=0;d<10;d++){
      var ph=reduced.matches?.34:((time*.72+d*.101)%1),side=d%2===0?-1:1,spread=(5+d*1.5)*s;
    ...
    for(var r=0;r<5;r++){
      var rp=reduced.matches?.38:((time*.245+r/5)%1),inner=r<2;
  ...
  function draw(time){
    ...
    var mobile=width<700,fallX=mobile?878:962,lipY=mobile?426:478,waterY=mobile?540:592;
  ...
  function resize(){
    ...
    ratio=Math.min(window.devicePixelRatio||1,1.45,Math.sqrt(1050000/(width*height)));scale=Math.max(width/W,height/H);left=(width-W*scale)*(width<600?.69:.5);
