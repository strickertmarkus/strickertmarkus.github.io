(function () {
  ...
  function renderActivity() {
    ...
    var values=buckets.map(function(b){return b[metric];}),max=Math.max.apply(Math,values.concat([metric==='minutes'?60:4])),width=clamp(byId('activity-chart').clientWidth||800,320,800),height=clamp(byId('activity-chart').clientHeight||180,164,195),left=48,right=20,top=26,bottom=38,plotW=width-left-right,plotH=height-top-bottom,step=plotW/buckets.length,barW=Math.min(18,step*.24);
    var svg='<svg viewBox="0 0 '+width+' '+height+'" preserveAspectRatio="none" aria-hidden="true"><defs><linearGradient id="bar-light" x1="0" y1="1" x2="0" y2="0"><stop stop-color="#ff657a" stop-opacity=".16"/><stop offset="1" stop-color="#ff9a91"/></linearGradient><filter id="bar-soft-glow" x="-80%" y="-30%" width="260%" height="160%" color-interpolation-filters="sRGB"><feGaussianBlur stdDeviation="3.1"/></filter></defs>';
