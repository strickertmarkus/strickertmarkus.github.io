...
old_ver = '20260915-compact-timer-center-v1'
new_ver = '20260915-first-paint-v1'
...
needle = "  if (!isExercise) return;\n\n  var pulseDefaultBoot = true;"
insert = """  if (!isExercise) return;
  ...
  var pulseDefaultBoot = true;"""
if needle not in s:
    raise SystemExit('auth-config first-paint insertion anchor missing')
s = s.replace(needle, insert, 1)

old_pulse = """        'html.exercise-concept-pulse-home-v1,html.exercise-concept-pulse-home-v1 body{background:#080D14!important}' +
...
new_pulse = """        'html.exercise-concept-pulse-home-v1,html.exercise-concept-pulse-home-v1 body{background:#080D14!important}' +
        'html.exercise-concept-pulse-home-v1 body .app-wrap{visibility:visible!important;opacity:1!important}' +
...
if old_pulse not in s:
    raise SystemExit('auth-config pulse preboot visibility anchor missing')
s = s.replace(old_pulse, new_pulse, 1)

old_override = "      'html.exercise-concept-pulse-home-v1.exercise-shell-booting-v13 body .main-content,html.exercise-concept-pulse-home-v1.exercise-shell-booting-v13 body .fab{visibility:visible!important;opacity:1!important}' +\n"
if old_override not in s:
    raise SystemExit('auth-config legacy visible override missing')
s = s.replace(old_override, '', 1)

old_reveal = """  function revealFinalFrame() {
...
new_reveal = """  var exerciseDomReadyV1 = document.readyState !== 'loading';
...
if old_reveal not in s:
    raise SystemExit('auth-config reveal block missing')
s = s.replace(old_reveal, new_reveal, 1)
p.write_text(s)

p = Path('budget/auth-gate.js')
s = p.read_text()
old_loader = """    (function loadExerciseAt(index) {
...
new_loader = """    (function loadExerciseAt(index) {
...
if old_loader not in s:
    raise SystemExit('auth-gate exercise loader anchor missing')
